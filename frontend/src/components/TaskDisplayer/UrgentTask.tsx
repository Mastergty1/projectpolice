...
export default function UrgentTask() {
    ...
    useEffect(() => {
        const fetchUrgentTasks = async () => {
            try {
                const cacheKey = 'urgentTasksCache';
                const timeKey = 'urgentTasksCacheTime';
                const cachedData = sessionStorage.getItem(cacheKey);
                const cacheTime = sessionStorage.getItem(timeKey);

                if (cachedData) {
                    setTasks(JSON.parse(cachedData));
                    setIsLoading(false); // ทันทีทันใด
                    // ถ้าข้อมูลเพิ่งโหลดมาไม่เกิน 30 วินาที ไม่ต้องดึงใหม่
                ...
                if (!sessionStorage.getItem('urgentTasksCache')) setTasks(initialTaskData);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleStatusChange = async (id: string, newStatus: TaskStatus) => {
        try {
            ...
            setTasks((prevTasks) => {
                ...
                sessionStorage.setItem('urgentTasksCache', JSON.stringify(updatedTasks));
                return updatedTasks;
            });
